class ApplyPage {
  ...
  setupCheckList(checklist) {
    ...
    checklist.forEach(item => {
      cy.$_GetTableRow(item, 'sigv-check-list').find('.p-checkbox').then($checkbox => {
  ...
  setupTermsAndConditions() {
    ...
    cy.$_GetTableRow('Commission', 'p-accordiontab').find('.p-checkbox').then($checkbox => {
      if ($checkbox.hasClass('p-checkbox-checked')) {
        return false;
      }
      cy.wrap($checkbox).click();
  ...
  setupDisbursementInformation() {
    ...
    cy.contains('p-checkbox', 'Remit').find('.p-checkbox').then($checkbox => {
      if ($checkbox.hasClass('p-checkbox-checked')) {
        cy.get('sigv-disb-remit').find('p-tableheadercheckbox').click();
